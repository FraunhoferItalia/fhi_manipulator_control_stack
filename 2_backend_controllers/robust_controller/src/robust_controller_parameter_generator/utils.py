...
def get_subtree(tree: Element, subtree_name: List[str]) -> Element:
    for child in tree:
...
def InertiaMatFromVect(inertiaVect):  #  Ixx, Iyy, Izz, Ixy, Ixz, Iyz
    # function to build inertia matrix from inertia vector
    InertiaMat = np.array(
        [
            [inertiaVect.item(0), inertiaVect.item(3), inertiaVect.item(4)],
            [inertiaVect.item(3), inertiaVect.item(1), inertiaVect.item(5)],
            [inertiaVect.item(4), inertiaVect.item(5), inertiaVect.item(2)],
        ]
    )
    return InertiaMat


def InertiaVectFromMat(inertiaVect):  #  Ixx, Iyy, Izz, Ixy, Ixz, Iyz
    # function to build inertia matrix from inertia vector
    InertiaVect = [
        inertiaVect[0][0],
        inertiaVect[1][1],
...
def newton_euler_urdf(q, dq, ddq, robot_parameters):
    ...
    I = np.zeros((k, 3, 3))
    for i in range(k):
        I[i, :, :] = InertiaMatFromVect(np.array(robot_parameters.DynPar.I)[i, :])
